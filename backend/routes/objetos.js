...
router.put('/:id', async (req, res) => {
  try {
    ...
    const [existingObject] = await db.query('SELECT * FROM objeto WHERE id = ?', [id]);
    ...
    await db.query(
      `UPDATE objeto SET ${updates.join(', ')} WHERE id = ?`,
      values
    );
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await db.query('DELETE FROM objeto WHERE id = ?', [id]);
