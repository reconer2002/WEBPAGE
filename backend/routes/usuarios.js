...
router.patch('/:id/rol', verifyToken, verifyPermiso('moderar_usuarios'), async (req, res) => {
  ...
  if (!rol) return res.status(400).json({ mensaje: 'El campo rol es obligatorio.' });

  try {
    const [rolExistente] = await req.db.query('SELECT id FROM roles WHERE nombre = ?', [rol]);
    if (rolExistente.length === 0) return res.status(400).json({ mensaje: 'El rol especificado no existe.' });
    ...
    const [usuarioActualizado] = await req.db.query(
      `SELECT u.id, u.nombre, u.email, r.nombre AS rol
       FROM usuarios u
       INNER JOIN roles r ON u.rol_id = r.id
       WHERE u.id = ?`,
...
router.patch('/roles', verifyToken, verifyPermiso('moderar_usuarios'), async (req, res) => {
  try {
    ...
    if (!Array.isArray(ids) || ids.length === 0) return res.status(400).json({ error: 'Debes enviar un array con IDs de usuarios.' });
    if (!rol || typeof rol !== 'string') return res.status(400).json({ error: 'Debes enviar un rol válido.' });

    const [rowsRol] = await req.db.execute('SELECT id FROM roles WHERE nombre = ?', [rol]);
    if (rowsRol.length === 0) return res.status(400).json({ error: `El rol '${rol}' no existe.` });

    const rolId = rowsRol[0].id;
    const idsInt = Array.from(new Set(ids.map(i => parseInt(i, 10)).filter(n => Number.isInteger(n) && n > 0)));
    if (idsInt.length === 0) return res.status(400).json({ error: 'No se encontraron IDs válidos en el array.' });
    ...
    if (result.affectedRows === 0) return res.status(404).json({ error: 'No se actualizaron usuarios (IDs no encontrados).' });

    const [usuariosActualizados] = await req.db.execute(
      `SELECT u.id, u.nombre, u.email, r.nombre AS rol
       FROM usuarios u
       INNER JOIN roles r ON u.rol_id = r.id
...
router.post('/', verifyToken, verifyPermiso('moderar_usuarios'), async (req, res) => {
  ...
  try {
    await req.db.execute('INSERT INTO usuarios (nombre, email, password) VALUES (?, ?, ?)', [nombre, email, hash]);
...
router.put('/:id', verifyToken, verifyPermiso('moderar_usuarios'), async (req, res) => {
  ...
  try {
    await req.db.execute('UPDATE usuarios SET nombre = ?, email = ? WHERE id = ?', [nombre, email, req.params.id]);
